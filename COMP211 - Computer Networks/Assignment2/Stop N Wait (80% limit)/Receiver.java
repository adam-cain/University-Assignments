...

{
    ...
    protected void Input(Packet p) {
        // If cant verify the checksum of packet return
        System.out.println("Recieved: "+p);
        System.out.println("Expected:"+expectedSeq + "-- Got: " +p.getSeqnum());
        if (!Sender.verifyCheckSum(p)) {
            return;
        }
        System.out.println("...Checksum Verified");
        Packet newP = new Packet(0, p.getSeqnum(), 0);
        if (p.getSeqnum() == expectedSeq) {
        System.out.println("...Sequence number matched");

            deliverData(p.getPayload());
            expectedSeq++;
        }
        udtSend(Sender.applyCheckSum(newP));
        System.out.println("");
