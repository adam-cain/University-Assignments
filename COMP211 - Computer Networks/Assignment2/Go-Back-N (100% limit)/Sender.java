...

{
    ...
    protected void Input(Packet p) {
        ...
        //complicates algorithim if NACK request is corrupted
        //will be caught by sequence diffrence step
        if (Sender.verifyCheckSum(p)) {
            udtSend(getBuffer(p.getAcknum()));
            return;
        }
        System.out.println("Corrupted packet recieved at sender");
    }
